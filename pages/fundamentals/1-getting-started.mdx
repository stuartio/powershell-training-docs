...
> Unless you have updated your local preferences, the PowerShell Gallery is not a trusted source, and you will be prompted to confirm that you wish to install the module. You can avoid this prompt by running `Install-PSResource -TrustRepository` to avoid the prompt, or `Set-PSRepository -Name PSGallery -InstallationPolicy Trusted` to mark the gallery as a trusted source for all future installs. Alternatively, if you are using `Install-Module` you can use the `-Force` parameter to avoid the prompt.
